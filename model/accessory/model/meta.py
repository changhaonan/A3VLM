...
class MetaModel(nn.Module):
    def __init__(
        ...
        with_visual: bool = False, max_seq_len: int = 4096, pretrain_stage=False,
    ) -> None:
        super().__init__()
        ...
        if "internlm" not in llama_type:
            llama_args: ModelArgs = ModelArgs(**llama_args)
        
        else:
            print(f"Load tokenizer_path from {tokenizer_path} with vocab_size {tokenizer.n_words}")
            # llama_args: ModelArgs = ModelArgs(**llama_args)
            llama_args: ModelArgs = dacite.from_dict(ModelArgs, llama_args)
        ...
        print(f"Model: {llama_type} with pretrain_stage: {pretrain_stage}")        
        self._set_default_trainability(pretrain_stage)
    ...
    def get_trainable_params(self, pretrain_stage=False):
        llma_trainable = self.llma.get_trainable_params(pretrain_stage)
        return {"llma." + name: param for name, param in llma_trainable.items()}

    def _set_default_trainability(self, pretrain_stage=False):
        for key, value in self.named_parameters():
            value.requires_grad = False
        for key, value in self.get_trainable_params(pretrain_stage=pretrain_stage).items():
            value.requires_grad = True

    def forward(self, examples, labels, images=None, depth_imgs=None):
        with torch.no_grad():
    ...
    def generate(
        ...
        if depth_images is not None:
            depth_images = depth_images.to(list(self.parameters())[0].device)
        ...
        if images is not None:
            max_seq_len -= self.llma.image_words # 1186 for 2 images input
        ...
        for cur_pos in range(start_pos, total_len):
            if depth_images is not None:                
                if prev_pos == 0:
                    logits = self.llma.forward_inference(tokens[:, prev_pos:cur_pos], prev_pos, images, depth_images).float()
                else:
                    logits = self.llma.forward_inference(tokens[:, prev_pos:cur_pos], prev_pos).float()
            else:
    ...
    def get_basic_block_classes(self):
        if hasattr(self.llma, 'get_basic_block_classes'):
            return self.llma.get_basic_block_classes()
        else:
            return [type(self.llma.layers[0])]