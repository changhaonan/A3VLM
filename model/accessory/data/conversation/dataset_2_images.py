...
class FinetuneDialogDataset(Dataset):
    ...
    def get_item_func(self, index):
        ...
        filename = data_item['depth_image']
        depth_image = read_img_general(filename)
        ...
        return input2, labels, input2_mask, image, depth_image
...
if __name__ == '__main__':
    # test the dataloader
    yaml_path = "/mnt/lustre/huangsiyuan/LLaMA2-Accessory/accessory/configs/data/finetune/mm/affordance_rgb_with_depth.yaml"
    from accessory.data.transform import get_transform
    transform=get_transform("random_resized_crop", 448)
    dataset = FinetuneDialogDataset(yaml_path, transform, max_words=4096)
    print(dataset[0])
