...
class FinetuneDatasetPoint(Dataset):
    def __init__(self, config_path, transform, max_words=30, image_words=257, tokenizer=None,
        ...
        if need_collect_anno:
            ...
            for meta in self.config['META']:
                ...
                assert meta_type in ["text", "image_text", "point"]
                ...
                # add meta_type to each data item
                for data_item in meta_l:
                    data_item['meta_type'] = meta_type
        ...
        assert self.image_words is not None, "image_words should not be None"
    ...
    def __getitem__(self, index):
        ...
        meta_type = data_item['meta_type']
        image = data_item.get("image", None)
        if image is not None:
            if meta_type == "image_text":
                image = Image.open(image).convert('RGB')
                # warnings.warn("image channel format: BGR")
                # image = Image.fromarray(cv2.imread(image))
                image = self.transform(image)
            elif meta_type == "point":
                image = load_objaverse_point_cloud(image, use_color=True) # objvarse has already been converted into 8192 points
                image = torch.from_numpy(image.astype(np.float32))
            else:
                raise NotImplementedError(f"Unknown meta_type: {meta_type}")
        ...
        if image is not None:
            max_words = self.max_words - self.image_words[meta_type]
        else:
...
class FinetuneDistSampler(Sampler):
    ...
    def __init__(self, dataset: FinetuneDatasetPoint, num_replicas: Optional[int] = None,
